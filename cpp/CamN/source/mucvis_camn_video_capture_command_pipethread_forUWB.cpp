...
#define WIDTH "800"  // 映像幅 1280 1920 
#define HEIGHT "600"  // 映像高さ 720 1080
...
public:
    ...
    int start_send() {
        ...
        //hr_clock::time_point target_time;
        ...
        while (true) {
            ...
            while (hr_clock::now() < target_time) {
                // OSのタイマーに頼らず、CPUを回して正確に待機する
            }


		// OSの1msタイマー精度の壁を越えるための、超高精度ビジーウェイト（スピンロック）
            //target_time = ipt_start + std::chrono::duration<double>(ipt_interval);
            // ★変更点：sleep_until をやめて、時間になるまで whileループ で正確に待機する
            //while (hr_clock::now() < target_time) {
	//	}
                // 何もせずに待機（CPUを1コア消費しますが、マイクロ秒単位で完璧なIPTを実現します）
            // ipt_interval待機
...
int main(int argc, char* argv[]) {
    ...
    if (motor_pid == -1) {
        ...
        execl(motor_program_path, "main", control_mode.c_str(), (char *)NULL);
    ...
    if (pid == 0) {
        ...
        std::string cmd = 
            ...
            " --codec h264 --inline --intra 10 -o - | "  // libcamera-vidの出力をパイプ

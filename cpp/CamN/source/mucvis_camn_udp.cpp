...
#define FILE_NAME_PREFIX "videos/"  // 映像ファイル名プレフィックス
...
std::string g_mode;  // モード: UDP/RTS

const double generate_time_all = 30.0;  // ビデオデータ生成時間 [s]
...
public:
    Mucvis_camn(std::string my_address, int my_port, std::string down_address, int down_port, Log& log, double ipt_interval, hr_clock::time_point hr_start_time, int pipefd[2], std::string video_file_name) {
        ...
        // 映像ファイルを開く
        // std::string file_name_pass = FILE_NAME_PREFIX + g_mode + "_" + video_file_name + "/" + video_file_name + ".ts";
    ...
    Packet make_packet() {
        ...
        if (packet_type == TYPE_VIDEO) {
            ...
            while (video_data.size() == 0) {
                ...
                // 映像データキューが空の場合は，epollでg_video_bytequeueを監視し，映像データキューに取り出す
                if (video_data.empty()) {
                    // ITP_interval待機
                    std::this_thread::sleep_for(std::chrono::duration<double>(ipt_interval));
                } else {
                    break;
                }
                ...
                //                 if (g_video_bytequeue.size() < VIDEO_BUFFER_SIZE * 2) {
                //                     // std::cout << "video_bytequeue_size = " << g_video_bytequeue.size() << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    // 引数チェック
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " [My IP Address] [Send IP Address] [IPT interval(s)] [UDP/RTS] [log_name]" << endl;
        return 1;
    }
    ...
    double ipt_interval = 0;  // 送信間隔 1msぐらい?
    g_mode = argv[3];  // モード: UDP/RTS
    ...
    // }
    // if (!std::filesystem::exists(FILE_NAME_PREFIX + g_mode + "_" + video_file_name)) {
    //     std::filesystem::create_directory(FILE_NAME_PREFIX + g_mode + "_" + video_file_name);
    // }

    // ログファイル作成
    Log log(g_mode, argv[5], system_start_time);
    log.write("CamN");
    log.write("ipt_interval = " + std::to_string(ipt_interval) + " s");
    log.write("mode = " + g_mode);
    ...
    std::cout << "video_bit_rate = " + std::to_string(video_bit_rate) + " Mbps" << std::endl;
...
void generate_video_fixed_interval(double generate_time_all, double video_bit_rate, Log& log, hr_clock::time_point hr_start_time) {
    ...
    while (true) {
        ...
        if (duration.count() >= stop_time) {
            std::this_thread::sleep_for(std::chrono::milliseconds(100));  // 映像生成後0.1秒待ったあと，終了
            return;
        }

...
std::mutex g_lock;
bool g_wait_recv = false;  // 受信中かどうかを示すフラグ
...
public:
    ...
    Packet make_packet_down_receiver() {
        uint32_t packet_type = TYPE_DUMMY;
        while (packet_type == TYPE_DUMMY ) {
            g_lock.lock();
    ...
    void down_receiver_recv() {
        ...
        // // 下りへ送信(UDP版)
        // sendto(down_receiver_sock, payload.data(), payload.size(), 0, (struct sockaddr *)&down_addr, sizeof(down_addr));
        ...
        // log->write_rn(duration, "Send", packet_type, "Down", seq, payload.size(), video_packet_queue_size);
    }
    ...
    void up_receiver_recv() {
        ...
        // // 上りへ送信(UDP版)
        // sendto(up_receiver_sock, payload.data(), payload.size(), 0, (struct sockaddr *)&up_addr, sizeof(up_addr));
        ...
        //     seq = packet.get_commandSeq();
        // }
        // log->write_rn(duration, "Send", packet_type, "Up", seq, payload.size(), video_packet_queue_size);
        // g_lock.lock();
        // m_ack = packet.get_ack();
        // g_lock.unlock();
    }
...
int main(int argc, char* argv[]) {
    ...
    // 引数チェック ローカル用
    if (argc != 6) {
        std::cerr << "Usage: " << argv[0] << " [My IP Address] [Up IP Address] [Down IP Address] [UDP/RTS] [log_name]" << std::endl;
        return 1;
    }
    ...
    std::string mode = argv[4];  // モード: UDP/RTS

    // ログファイルの作成
    Log log(mode, argv[5], system_start_time);
    log.write("RN");
    log.write("ipt_interval = " + std::to_string(ipt_interval) + " s");
    log.write("mode = " + mode);

    cout << "RN" << std::endl;
    cout << "ipt_interval = " + std::to_string(ipt_interval) + " s" << std::endl;
    ...
    // UDP送信スレッド開始
    std::thread udp_send_thread(&Mucvis_rn::udp_send_start, &mucvis_rn);

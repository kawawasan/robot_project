...
void generate_command_from_input(Log& log, hr_clock::time_point hr_start_time, const int node_num, const double communication_range, const double Vehicle_body_length, std::vector<std::vector<std::string>>& routing_table) {
    ...
    double correction_distance = (node_num - 1) * Vehicle_body_length;

    while (generate_input_end == false) {
        std::string command_data;
        std::cout << "Enter control command (or 'exit' to quit): " << std::endl;
        std::cout << "(Input '0' for MAX hop return, or distance in meters)" << std::endl;
        ...
        // --- 0入力時の特殊処理 ---
        if (command_data == "0") {
            std::cout << "[Return Mode] Setting to MAXIMUM hop (adjacent relay)." << std::endl;
            for (int i = 0; i < node_num; i++) node_position[i] = 0;

            // 隣接ノードへのリレーを強制設定
            routing_table[0][3] = "2"; // CamN -> Node 2
            for (int i = 1; i < node_num - 1; i++) {
                routing_table[i][2] = std::to_string(i);      // Up: 前へ
                routing_table[i][3] = std::to_string(i + 2);  // Down: 次へ
            }
            routing_table[node_num - 1][2] = std::to_string(node_num - 1); // CN -> 前のRNへ
            
            g_send_num.store(node_num - 1); // 送信先を直近のRNに更新
        } 
        // --- 通常の距離計算（既存ロジック） ---
        else {
            try {
                input_distance = std::stod(command_data) - correction_distance;
                // ここから元の計算ロジックをそのまま維持
                for (int i = 0; i < node_num; i++) {
                    double d = (i == 0) ? input_distance : input_distance;
                    if (d < 0) d = 0;
                    node_position[i] = (communication_range > d) ? d : communication_range;
                    input_distance -= node_position[i];
                    node_cn_distance[i] = node_position[i] + (node_num - i - 2) * Vehicle_body_length;
                }

                // ルーティング計算（既存の条件分岐）
                if (node_cn_distance[0] > communication_range) routing_table[0][3] = "2";
                else routing_table[0][3] = std::to_string(node_num);

                for (int i = node_num - 3; i >= 0; i--) {
                    if (node_cn_distance[i] > communication_range) {
                        routing_table[node_num-1][2] = std::to_string(i + 2);
                        break;
                    } else routing_table[node_num-1][2] = "1";
                }

                for (int i = 1; i < node_num - 1; i++) {
                    routing_table[i][2] = (node_cn_distance[i-1] > communication_range) ? std::to_string(i) : "0";
                    if (node_num - i - 1 > 1 && node_cn_distance[i] > communication_range) {
                        routing_table[i][3] = std::to_string(i + 2);
                    } else {
                        routing_table[i][3] = std::to_string(node_num);
                    }
                }
                g_send_num.store(std::stoi(routing_table[node_num - 1][2]));
            }
        ...
        // --- コマンド文字列生成と送信（0入力・通常入力共通） ---
        command_data.clear();
        std::vector<int> node_pos_cm(node_num);
        for (int i = 0; i < node_num; i++) {
            node_pos_cm[i] = static_cast<int>(std::round(node_position[i] * 100));
        }

        command_data = std::to_string(node_pos_cm[0]) + " " + routing_table[0][3] + ",";
        for (int i = 1; i < node_num - 1; i++) {
            command_data += std::to_string(node_pos_cm[i]) + " " + routing_table[i][2] + " " + routing_table[i][3] + ",";
        }
        ...
        log.write_generate(std::chrono::duration<double>(hr_clock::now() - start_time), "Command", generate_num++, command_data.size(), command_data);

...
public:
    ...
    Packet make_packet() {
        ...
        // コマンドが無いとき生成されるまで待機(UDP版)
        while (g_command_queue_size == 0) {
...
int main(int argc, char* argv[]) {
    ...
    // 引数チェック
    if (argc != 5) {
        std::cerr << "Usage: " << argv[0] << " [My IP Address] [Send IP Address] [UDP/RTS] [log_name]" << endl;
        return 1;
    }
    ...
    // ログファイル作成
    Log log(g_mode, argv[4], system_start_time);
    log.write("CN");
    ...
    // 映像データ送信スレッド開始
    std::thread sender_thread(&Mucvis_cn::start_send, &mucvis_cn);

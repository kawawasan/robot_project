...
def main():
    if len(sys.argv) < 2:
        print("Usage: python3 analyze_control_packet.py <logs...>")
        return

    nodes = parse_logs_with_sync(sys.argv[1:])

    # ---------------------------------------------------------
    # 1. あなたのロジックを完全継承：VIDEOパケットから現在のフェーズを自動判定
    # ---------------------------------------------------------
    ...
    print("\n" + "="*95)
    print(" 📊 制御コマンド (Control Plane) 自動フェーズ別 【詳細統計評価表】")
    print("="*95)
    print(f"| {'Phase (Routing)':<16} | {'Sent':>5} | {'Recv':>5} | {'Loss(%)':>7} | {'Avg(ms)':>8} | {'Median(ms)':>10} | {'Std(ms)':>8} | {'Var':>10} |")
    print("-" * 95)
    ...
    for phase_name in ['1-Hop (Direct)', '2-Hop (RN2)', '3-Hop (RN1)']:
        ...
        if len(data['delays']) > 0:
            ...
            median_d = np.median(data['delays'])
            std_d = np.std(data['delays'])
            var_d = np.var(data['delays'])
            
            print(f"| {phase_name:<16} | {sent:>5} | {recv:>5} | {loss_rate:>6.1f}% | {avg_d:>8.1f} | {median_d:>10.1f} | {std_d:>8.1f} | {var_d:>10.1f} |")
        else:
            print(f"| {phase_name:<16} | {sent:>5} | {recv:>5} | {loss_rate:>6.1f}% | {'N/A':>8} | {'N/A':>10} | {'N/A':>8} | {'N/A':>10} |")
    print("="*95 + "\n")
